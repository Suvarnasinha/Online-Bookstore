...
const EditUser = () => {
    const { user1 } = useAuthContext(); // Get the user from the AuthContext
    const { id } = useParams();
  const [roles, setRoles] = useState([]);
  const [user, setUser] = useState();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    // Check if user is authenticated before fetching user details
    if (user) {
      getUserById();
    }
  }, [id, user]);
  ...
  const onSubmit = (values) => {
    ...
    UserService.update(updatedValue)
      .then((res) => {
        if (res) {
         ...
         {({
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-row-wrapper">
                ...
                {values.id !== user1.user.id && (
                  <div className="form-col">
                    <FormControl
                      className="dropdown-wrapper"
                      variant="outlined"
                      disabled={values.id === user1.user.id}
                    >
                      <InputLabel htmlFor="select">Roles</InputLabel>
                      <Select
                        name="roleId"
